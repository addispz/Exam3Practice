...
def practice_problem4c(points):
    ...
    for k in range(len(points)):
        ...
        if is_prime(current_point.x) and is_prime(current_point.y):
            original_x = current_point.x

            current_point.x = current_point.y
            current_point.y = original_x
...
def practice_problem4d(sequence):
    ...
    sum = 0
    for k in range(len(sequence) - 1):
        if is_prime(sequence[k]) == True and is_prime(sequence[k + 1]) == True:
            if sequence[k] != sequence[k + 1]:
                sum = sum + sequence[k]
    return sum
